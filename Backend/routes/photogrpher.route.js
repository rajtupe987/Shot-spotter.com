...
photographerRouter.get(
  "/",
  authenticate,
  athorization(["client", "admin", "photographer"]),
  async (req, res) => {
...
photographerRouter.get("/:studio_id",authenticate,athorization(["client", "admin", "photographer"]), async (req, res) => {
    const {studio_id} = req.params;
    try {
      const photographer = await Photographer.findById(studio_id);
...
photographerRouter.put(
  ...
  async (req, res) => {
    try {
      const photographer = await Photographer.findByIdAndUpdate(req.params.id, {
